...
def ml_dtree(
    ...
    recall = recall_score(y, pred, pos_label = 'Yes')
    precision = precision_score(y, pred, pos_label = 'Yes')

    return [clf, pred, accuracy, recall, precision]
...
def main():
    ...
    if choice == 'グラフ表示':
        ...
        activities2 = ["退職", "年齢", "出張頻度", "通勤距離", "最終学歴", "社員番号", "職場環境の満足度", "労働意欲", "査定ランク", "仕事への満足度", "月給(ドル)", "転職回数", "昇給率", "能力評価", "人間関係の満足度", "ストックオプション", "社会人歴", "昨年の研修時間", "ワークライフバランス", "在職年数", "現在の役職年数", "昇進からの経過年数", "現在の上司になってからの年数", "残業_有", "性別_男性", "配偶者_離婚", "配偶者_結婚", "配偶者_独身", "部署_人事", "部署_研究開発", "部署_営業", "専攻_経営", "専攻_科学", "専攻_医学", "専攻_工学", "専攻_人事", "専攻_その他", "役職_経営層", "役職_管理職", "役職_主任", "役職_一般"]
        choice2 = st.sidebar.selectbox("Select Activity", activities2)
    ...
    if choice == '学習と検証':
        option = st.sidebar.selectbox('select box:', [1, 2, 3, 4, 5])
        ...
        if 'df' in st.session_state:
            ...
            # 決定木による予測
            clf, train_pred, accuracy_score, recall_score, precision_score = ml_dtree(train_X, train_Y, option)

            # 決定木のツリーを出力
            st.write("決定木の可視化")
            st_display_dtree(clf, ["年齢", "出張頻度", "通勤距離", "最終学歴", "社員番号", "職場環境の満足度", "労働意欲", "査定ランク", "仕事への満足度", "月給(ドル)", "転職回数", "昇給率", "能力評価", "人間関係の満足度", "ストックオプション", "社会人歴", "昨年の研修時間", "ワークライフバランス", "在職年数", "現在の役職年数", "昇進からの経過年数", "現在の上司になってからの年数", "残業_有", "性別_男性", "配偶者_離婚", "配偶者_結婚", "配偶者_独身", "部署_人事", "部署_研究開発", "部署_営業", "専攻_経営", "専攻_科学", "専攻_医学", "専攻_工学", "専攻_人事", "専攻_その他", "役職_経営層", "役職_管理職", "役職_主任", "役職_一般"])

            # 正解率を出力
            st.write("決定木の予測")
            st.header("正解率:" + str(accuracy_score))
            st.header("再現率:" + str(recall_score))
            st.header("適合率:" + str(precision_score))
